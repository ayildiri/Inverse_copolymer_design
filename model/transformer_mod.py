...
class TransformerDecoder(TransformerDecoderBase):
    ...
    def forward(self, tgt, enc_out=None, step=None, **kwargs):
        ...
        # Customized GV
        if kwargs["add_latent"]:
            emb = self.embeddings(tgt, step=step, latent=enc_out)
        else:
        ...
        for layer in self.transformer_layers:
            dec_out, attn, attn_align = layer(
                ...
                add_latent=kwargs["add_latent"], #Customized GV
            )
...
class TransformerLMDecoder(TransformerDecoderBase):
    ...
    def forward(self, tgt, enc_out=None, step=None, **kwargs):
        ...
        # Customized GV
        if kwargs["add_latent"]:
            dec_out = self.embeddings(tgt, step=step, latent=enc_out)
        else:
